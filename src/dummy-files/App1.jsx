...
function TableWithCheckboxes() {
  ...
  const divideIntoPackages = (items, maxPrice) => {
    // Sort items by weight in descending order for more even distribution
    ...
    // Balance weights between packages
  ...
  const balancePackageWeights = (packages) => {
    const averageWeight =
      packages.reduce((total, pkg) => {
        return (
          total +
          pkg.reduce((totalWeight, item) => totalWeight + item.weight, 0)
        );
      }, 0) / packages.length;
    ...
    // Attempt to balance weights across all packages
    for (let i = 0; i < balancedPackages.length; i++) {
      ...
      while (
        ...
        for (let j = 0; j < currentPackage.package.length; j++) {
          ...
          if (
            currentPackage.totalWeight - item.weight >= averageWeight &&
  ...
  const calculateShippingCost = (pkg) => {
    ...
    for (const charge of weightCharges) {
      if (totalWeight >= charge.minWeight && totalWeight <= charge.maxWeight) {
          ...
          {items.map((item, index) => {
            ...
            return (
              <tr key={index}>
                ...
                <td>
                  <label htmlFor={checkboxId}>{`$${item.price}`}</label>
                </td>
                <td>
                  <label htmlFor={checkboxId}>{`${item.weight}g`}</label>
                </td>
              </tr>
            );
          })}
        ...
        {packages.length > 0 ? (
          packages.map((pkg, pkgIndex) => (
            <div key={pkgIndex} style={{ marginBottom: "10px" }}>
              ...
              <strong>
                Total Price: $
                {pkg.package.reduce((total, item) => total + item.price, 0)} -
                Total Weight:
                {pkg.package.reduce((total, item) => total + item.weight, 0)}g -
                Shipping Cost: ${pkg.shippingCost}
              </strong>
            </div>
          ))
        ) : (
